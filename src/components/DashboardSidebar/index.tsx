...
export default function Sidebar({ isSyllabusEnabled, isOpen, toggleSidebar }: SidebarProps) {
  const categoryName = isSyllabusEnabled ? Cookies.get("category_name") : "No syllabus selected";
        ...
        <div className="px-4 py-6">
          ...
          <div className="mt-8">
            ...
            <nav className="mt-4 space-y-2">
              <Link href={isSyllabusEnabled ? "/dashboard" : ""} className={`flex items-center px-4 py-2 rounded-md transition ${isSyllabusEnabled ? "hover:bg-gray-700" : "cursor-not-allowed"}`}> 
                <FiHome className="mr-3" /> Dashboard
              </Link>
              <Link href={isSyllabusEnabled ? "/dashboard/exams" : ""} className={`flex items-center px-4 py-2 rounded-md transition ${isSyllabusEnabled ? "hover:bg-gray-700" : "cursor-not-allowed"}`}>
                <FiList className="mr-3" /> Exams
              </Link>
              <Link href={isSyllabusEnabled ? "/dashboard/quizzes" : ""} className={`flex items-center px-4 py-2 rounded-md transition ${isSyllabusEnabled ? "hover:bg-gray-700" : "cursor-not-allowed"}`}>
                <FiCheckSquare className="mr-3" /> Quizzes
              </Link>
              <Link href={isSyllabusEnabled ? "/dashboard/learn-practice" : ""} className={`flex items-center px-4 py-2 rounded-md transition ${isSyllabusEnabled ? "hover:bg-gray-700" : "cursor-not-allowed"}`}>

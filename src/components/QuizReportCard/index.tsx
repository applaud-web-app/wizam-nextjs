...
// quiz Report PDF Component
const QuizReportPDF: React.FC<{ quizData: any }> = ({ quizData }) => {
  if (!quizData) {
    return <Text>No Data</Text>;
  }

  const { user_info, quiz_info, result_info } = quizData;
      ...
      <View style={styles.contentWrapper}>
          ...
          <View style={styles.infoSection}>
            ...
            <View style={styles.infoBlock}>
              ...
              <Text style={styles.infoText}>{quiz_info.name}</Text>
              <Text style={styles.label}>
                Completed on
              </Text>
              <Text style={styles.infoText}>{quiz_info.completed_on}</Text>
              <Text style={styles.label}>
                Session ID
              </Text>
              <Text style={styles.infoText}>{quiz_info.session_id}</Text>
            </View>
...
const quizReportGenerator: React.FC<{ uuid: string }> = ({ uuid }) => {
    ...
    const fetchquizData = async () => {
      try {
        const response = await axios.get(
          `${process.env.NEXT_PUBLIC_API_URL}/download-quiz-report/${uuid}`,
          {
            headers: {
        ...
        setError("Error fetching quiz report.");
        setLoading(false);
      }
    };
  ...
  const downloadPdf = async () => {
    if (quizData) {
      const doc = <QuizReportPDF quizData={quizData} />;
  ...
  if (!quizData) {
    return <div>Error loading quiz report</div>;
  }

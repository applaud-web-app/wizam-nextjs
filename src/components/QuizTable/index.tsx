    ...
    <div className="mb-8">
      <div className="flex justify-between items-center mb-3 flex-wrap">
        ...
        <Link
          ...
          className="text-quaternary font-semibold flex items-center space-x-2 hover:underline transition duration-200"
        >
          <span>See All</span>
          <FiArrowRight />
        </Link>
      ...
      <div className="overflow-x-auto rounded-lg shadow-sm">
        <table className="min-w-full table-auto rounded-lg overflow-hidden">
          ...
          {/* Add rounded bottom corners to tbody */}
          <tbody className="bg-white divide-y divide-gray-200 rounded-b-lg">
            {quizzes.map((quiz, index) => (
              <tr key={index} className="hover:bg-gray-50">
                ...
                <td className="p-4">
                  {quiz.duration_mode === "manual" ? quiz.duration : Math.floor(quiz.total_time / 60)} min
                </td>
                <td className="p-4">{quiz.total_questions}</td>
                <td className="p-4">
                  {quiz.point_mode == "manual" ? quiz.total_questions * quiz.point : quiz.total_marks}
                </td>
              </tr>
            ))}
          </tbody>

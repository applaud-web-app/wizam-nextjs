...
const QuizResult = ({ params }: QuizResultProps) => {
  ...
  useEffect(() => {
    ...
    const fetchQuizResults = async () => {
      try {
        const response = await axios.get(`${process.env.NEXT_PUBLIC_API_URL}/practice-set-result/${slug}`, {
          headers: {
...
// Time formatting function for minutes and seconds
    ...
    if (timeInMinutes === undefined || timeInMinutes === null) return "N/A";
   
    return `${timeInMinutes.toFixed(2)} min`; // Display minutes rounded to 2 decimal places
  };
